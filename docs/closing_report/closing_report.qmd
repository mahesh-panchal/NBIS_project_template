...
`/proj/naiss20xx-yy-zz/nobackup/nxf-work/apptainer-cache`. The container
image and version each Apptainer image was made from can be found in
the workflow's config files under `<project_root>/code/configs/`.

Custom container images are uploaded to `ghrc.io` (GitHub container registry) and associated
with the project repostory. Images hosted on `ghcr.io` are initially private if the code
repository is private. These images should be copied to a public respository on publication.
